...
simulated_params = run(input_parameters, batch_processing=False, check_time=100, parallel=8)
...
To avoid generating unnecessary data and to reduce the size of the generated data, you can control the logs generated by the simulation. The logs are controlled by annotations in the code. For example, to skip the log of a variable, you can use the `@Skip' annotation. You can find more information about the log controls in the [ReservedLogChannels.java](src/main/java/pol/log/ReservedLogChannels.java) file.
